...
async function loadDashboardStats() {
    try {
        ...
        // Get active sessions (demo data for now)
        // TODO: Implement actual session count from backend
        document.getElementById('active-sessions').textContent = Math.floor(users.length * 0.6);
...
function displayRecentUsers(users) {
    ...
    recentUsers.forEach(user => {
        const roleBadges = user.roles.map(role => {
            ...
            return `<span class="px-2 py-0.5 ${colorClass} text-xs font-semibold rounded-full">${escapeHtml(role.role_name)}</span>`;
        }).join(' ');
        ...
        html += `
            <div class="flex items-center justify-between p-3 bg-gray-900 rounded-lg border border-gray-700 hover:border-cyan-500/50 transition-colors">
                ...
                <div class="flex flex-col items-end space-y-1">
                    ...
                    <p class="text-xs text-gray-500">${getTimeAgo(user.created_at)}</p>
                </div>
            </div>
        `;
    });
...
function displayRecentActivity(requests) {
    ...
    // Combine requests with some demo activities
    const activities = [];
    ...
    requests.slice(0, 3).forEach(request => {
        activities.push({
            ...
            icon: 'clock',
            color: 'yellow',
            title: `${request.username} requested ${request.role_name} role`,
            time: request.created_at
        });
    });

    // Add demo activities (frontend only)
    activities.push({
        type: 'demo',
        icon: 'trophy',
        color: 'purple',
        title: 'New tournament "Summer Championship" created',
        ...
        html += `
            <div class="flex items-start space-x-3 p-3 bg-gray-900 rounded-lg border border-gray-700">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs text-gray-400 mt-1">${getTimeAgo(activity.time)}</p>
                </div>
            </div>
        `;
    });
