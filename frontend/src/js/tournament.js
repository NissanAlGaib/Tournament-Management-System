...
const TournamentAPI = {
    ...
    async getTournament(id) {
        try {
            const response = await fetch(`${this.baseURL}?action=tournament&id=${id}`);
            const data = await response.json();
            return data;
        } catch (error) {
    ...
    async getLeaderboard(tournamentId) {
        try {
            const response = await fetch(`${this.baseURL}?action=leaderboard&tournament_id=${tournamentId}`);
            const data = await response.json();
            return data;
        } catch (error) {
...
const TournamentUI = {
    ...
    renderTournamentCard(tournament) {
        ...
        const participantInfo = `${tournament.registered_participants || 0}/${tournament.max_participants || tournament.tournament_size}`;
        ...
        return `
            <div class="relative group">
                ...
                <div class="relative bg-gray-800 rounded-2xl border border-cyan-500/30 overflow-hidden">
                    ...
                    <div class="p-6">
                        <h3 class="text-xl font-bold text-white mb-2">${this.escapeHtml(tournament.name)}</h3>
                        <p class="text-gray-400 text-sm mb-4">${this.escapeHtml(tournament.description || 'No description')}</p>
                        ...
                        <button onclick="TournamentUI.viewTournament(${tournament.id})" class="w-full bg-gradient-to-r from-cyan-500 to-purple-600 hover:from-cyan-600 hover:to-purple-700 text-white font-bold py-3 rounded-xl transition-all duration-300 transform hover:scale-[1.02]">
                            View Details
                        </button>
                    </div>
                </div>
    ...
    getStatusBadge(status) {
        const badges = {
            ...
            'ongoing': '<span class="absolute top-4 right-4 px-3 py-1 bg-blue-500/90 text-white text-xs font-bold rounded-full">LIVE</span>',
            'completed': '<span class="absolute top-4 right-4 px-3 py-1 bg-gray-500/90 text-white text-xs font-bold rounded-full">COMPLETED</span>',
            'draft': '<span class="absolute top-4 right-4 px-3 py-1 bg-yellow-500/90 text-gray-900 text-xs font-bold rounded-full">DRAFT</span>',
            'cancelled': '<span class="absolute top-4 right-4 px-3 py-1 bg-red-500/90 text-white text-xs font-bold rounded-full">CANCELLED</span>'
        };
